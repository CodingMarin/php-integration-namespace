<?php
use controllers\controllerREG as registro;
require '../controllers/controllerREG.php';

$registro = new registro();
$error = array();
if(isset($_POST['submit'])) 
{
  $name     = $_POST['name'];
  $surname  = $_POST['surname'];
  $age      = $_POST['age'];
  $userid   = $_POST['username'];
  $pass[0]  = $_POST['password'];
  $pass[1]  = $_POST['passwordrepeat'];
  $email    = $_POST['email'];

  if ($pass[0]!=$pass[1]) {
    array_push($error,'The passwords do not match');

  } elseif (registro::issetRegistro($userid,'UserID')) {
    array_push($error,'The UserID'.$userid.' is already in use');

  }elseif (registro::issetRegistro($email, 'Email')) {
    array_push($error,'The Email '.$email.' is already in use');
  
  }elseif (strlen($pass[0]) < 3) {
    array_push($error,'The password is too short (6 chars min)');

  }elseif(strlen($userid) < 4){
    array_push($error,'The userid is too short (6 chars min)');
  
  }elseif (empty($name||$surname||$age||$userid)) {
    array_push($error,'Completa todos los campos');

  }else {
    $registro->register($userid,$name,$surname,$age,$email,$password);
  }
}
?>
<!DOCTYPE html>
<html lang="es" dir="ltr">

<head>
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0">
    <meta charset="utf-8">
    <link rel="stylesheet" type="text/css" href="css/main.css">
    <link href="css/css2.css" rel="stylesheet">
    <title>login</title>
</head>
<body>
    <div class="main">
        <div class="container a-container" id="a-container">
            <form class="form" id="b-form" action="sign-up" method="post">
                <h2 class="form_title title">CREATE ACCOUNT</h2>
                <div class="form__icons"><img class="form__icon"
                        src="data:image/svg+xml;base64,"
                        alt=""><img class="form__icon"
                        src="data:image/svg+xml;base64,"><img
                        class="form__icon"
                        src="data:image/svg+xml;base64,">
                </div>
                <span class="form__span warning">
                <?php
                  if (count($error) > 0) {
                    for ($i=0; $i <count($error); $i++) { 
                        echo $error[$i];
                    }    
                  }
                ?>
                </span>
                <a href=""></a>
                <input class="form__input" type="text" placeholder="Name" name="name" value="">
                <input class="form__input" type="text" placeholder="Surname" name="surname" value="">
                <input class="form__input" type="text" placeholder="Age" name="age" value="">
                <input class="form__input" type="text" placeholder="Username" name="username" value="">
                <input class="form__input" type="password" placeholder="Password" name="password" required value="">
                <input class="form__input" type="password" placeholder="Repeat Password" name="passwordrepeat" required
                    value="">
                <input class="form__input" type="email" placeholder="Email" name="email" required value="">
                <button value="" class="switch__button button" name="submit">REGISTER</button>
            </form>
        </div>
        <div class="switch" id="switch-cnt">
            <div class="switch__circle"></div>
            <div class="switch__circle switch__circle--t"></div>
            <div class="switch__container" id="switch-c1">
                <h2 class="switch__title title">Welcome Back !</h2>
                <p class="switch__description description">To keep connected with us please login with your personal
                    info</p>
                <button class="switch__button button switch-btn">SIGN IN</button>
            </div>
            <div class="switch__container is-hidden" id="switch-c2">
                <h2 class="switch__title title">Hello Friend !</h2>
                <p class="switch__description description">Enter your personal details and start journey with us</p>
                <button class="switch__button button switch-btn">More</button>
            </div>
        </div>
    </div>
    <script src="js/main.js"></script>
</body>
</html>